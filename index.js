...
export default function stringToReactElement(text, regexAndRendererPairs) {
  ...
  const newText = splitedText.map(splitItem => regexAndRendererPairs.find(regexAndRendererPairsItem => regexAndRendererPairsItem.regex.test(splitItem))?.renderer(splitItem));
  return newText;
}